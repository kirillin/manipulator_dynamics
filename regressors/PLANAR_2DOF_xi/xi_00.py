...
class Xi00:
	...
	def opL0(self, q, dq, ddq, a, d, theta):
		opL_0 = (-a[0]*sin(theta[0])**2 - a[0]*cos(theta[0])**2)*(-a[0]*sin(theta[0])**2*ddq[0]/2 - a[0]*cos(theta[0])**2*ddq[0]/2) + (-a[0]*sin(theta[0])**2/2 - a[0]*cos(theta[0])**2/2)*(-a[0]*sin(theta[0])**2*ddq[0] - a[0]*cos(theta[0])**2*ddq[0]) - 9.82*(a[0]*sin(theta[0])**2 + a[0]*cos(theta[0])**2)*cos(theta[0])
		return opL_0

	def opL1(self, q, dq, ddq, a, d, theta):
		opL_1 = a[0]*sin(theta[0])**2*ddq[0] + a[0]*cos(theta[0])**2*ddq[0] - (-a[0]*sin(theta[0])**2 - a[0]*cos(theta[0])**2)*ddq[0] - 9.82*cos(theta[0])
		return opL_1

	def opL2(self, q, dq, ddq, a, d, theta):
		opL_2 = 9.82*sin(theta[0])
		return opL_2
